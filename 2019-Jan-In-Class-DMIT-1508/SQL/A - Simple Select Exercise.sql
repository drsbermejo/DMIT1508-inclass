...
-- when refering to ALL DO NOT use *
SELECT ALL 
FROM Club
-- select statement produces a table of results which will consist of one or more rows of data
SELECT  ClubId, ClubName
FROM    Club

-- when selecting from an existing table, when identifying which columns are to be shown,
-- then those columns are used as the column title for the results table
-- as keyword used to assign a title to the column in the results table
...
FROM Course
WHERE Course.CourseName IN 'programming'

--12. Select all the ClubNames who start with N or C.
SELECT ALL ClubNames
FROM Club
WHERE Club.ClubName LIKE 'N%' AND 'C%'
...
SELECT StudentID
FROM Payment
WHERE Payment.Amount < 500 OR Payment.PaymentID = 5